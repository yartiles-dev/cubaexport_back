...
function processErrorsQuery(req, nameFolderModule, tableName) {
    ...
    if (req.body.hasOwnProperty('pagination')) {
        ...
        if (typeof pagination !== 'number') {
            error.push(`El parametro pagination debe ser un numero entero`)
        }
    }
...
function buildQueryFromReqFirstRequest(req, nameFolderModule, tableName) {
    var relations = relationsFirstRequest(req, nameFolderModule, tableName);
...
function buildQueryFromReqSecondRequest(req, nameFolderModule, tableName, models) {
    var relations = this.relations(req, nameFolderModule, tableName);
...
function buildQueryFromReq(req, nameFolderModule, tableName) {
    var relations = this.relations(req, nameFolderModule, tableName);
    if(relations === "Forbidden"){
        return relations
    }
...
function buildQueryFromReqSingleCollection(req, nameFolderModule, tableName) {
    var relations = this.relations(req, nameFolderModule, tableName);
    if(relations === "Forbidden"){
        return relations
    }
...
function relations(req, nameFolderModule, tableName) {
    let relations = [];
    var errorPermission = false
    if (req.body.relations) {
        if (req.body.relations === "all") {
                var ArrayRelationsModel = require('../../modules/' + nameFolderModule + '/models/' + tableName).arrayRelations
            ArrayRelationsModel.forEach(element => {
                if(!errorPermission){
                var RolePermissionAuthenticationNested = global.app.security.RolePermissionAuthenticationNested(element.nameModel, nameFolderModule, req.loggedUser.roleId);
                ...
                if (element.typeRelation === 'hasMany') {
                    relations.push({ association: element.nameRelation, attributes: RelationAtributos, subQuery: false, required: true, separate: isSeparate })
                } else if (element.typeRelation === 'hasOne' || element.typeRelation === 'belongsTo') {
                    relations.push({ association: element.nameRelation, attributes: RelationAtributos, subQuery: false, right: true, separate: isSeparate })
                }
                }
            }
            })
                ...
                if (obj.indexOf(".") !== -1) {
                    ...
                    for (let j = 0; j < array.length; j++) {
                        ...
                        for (let h = 0; h < ArrayRelationsModel.length && !find; h++) {
                            if (ArrayRelationsModel[h].nameRelation === array[j]) {
                                var RolePermissionAuthenticationNested = global.app.security.RolePermissionAuthenticationNested(ArrayRelationsModel[h].nameModel, nameFolderModule, req.loggedUser.roleId);
                                if(RolePermissionAuthenticationNested.error !== ""){
                                   errorPermission = true
                                   relations = "Forbidden"
                                } else {
                    ...
                    for (let j = array.length - 1; j >= 0; j--) {
                        ...
                        if (j === array.length - 1) {
                            if (arrayIshasMany[j].isHasOne || (!arrayIshasMany[j].isHasOne && !arrayIshasMany[j].isHasMany)) {
                                relation = { association: array[j], attributes: RelationAtributos, subQuery: false, rigth: true, separate: arrayIshasMany[j].isHasMany };
                            } else {
                                relation = { association: array[j], attributes: RelationAtributos, subQuery: false, required: true, separate: arrayIshasMany[j].isHasMany };
                            }
                        } else {
                            if (arrayIshasMany[j].isHasOne || (!arrayIshasMany[j].isHasOne && !arrayIshasMany[j].isHasMany)) {
                                relation = { association: array[j], attributes: RelationAtributos, subQuery: false, rigth: true, separate: arrayIshasMany[j].isHasMany, include: [relation] };
                            } else {
                                relation = { association: array[j], attributes: RelationAtributos, subQuery: false, required: true, separate: arrayIshasMany[j].isHasMany, include: [relation] };
                            }
                        }
                    }
                    ...
                    for (let h = 0; h < ArrayRelationsModel.length; h++) {
                        if (ArrayRelationsModel[h].nameRelation === obj) {
                            ...
                            if (ArrayRelationsModel[h].typeRelation === 'hasMany') {
                                isSeparate = true
                                relations.push({ association: obj, attributes: RelationAtributos, subQuery: false, required: true, separate: isSeparate });
                            } else if (ArrayRelationsModel[h].typeRelation === 'belongsTo') {
                                relations.push({ association: obj, attributes: RelationAtributos, subQuery: false, rigth: true, separate: isSeparate });
                            } else if (ArrayRelationsModel[h].typeRelation === 'hasOne') {
                                relations.push({ association: obj, attributes: RelationAtributos, subQuery: false, rigth: true, separate: isSeparate });
                            }                           
                        }
...
function relationsFirstRequest(req, nameFolderModule, tableName) {
    let relations = [];
    var errorPermission = false
    if (req.body.relations) {
        if (req.body.relations === "all") {
            ...
            ArrayRelationsModel.forEach(element => {
                if(!errorPermission){
                    ...
                    if(RolePermissionAuthenticationNested.error !== ""){
                        ...
                        if (element.typeRelation === 'hasMany') {
                            relations.push({ association: element.nameRelation, attributes: [], distinct: true, subQuery: false })
                        } else if (element.typeRelation === 'hasOne' || element.typeRelation === 'belongsTo') {
                            relations.push({ association: element.nameRelation, attributes: [], distinct: true, subQuery: false })
                        }
                    }
            ...
            for (let i = 0; i < req.body.relations.length; i++) {
                ...
                if (obj.indexOf(".") !== -1) {
                    ...
                    for (let j = array.length - 1; j >= 0; j--) {
                        if (j === array.length - 1) {
                            if (arrayIshasMany[j].isHasOne || (!arrayIshasMany[j].isHasOne && !arrayIshasMany[j].isHasMany)) {
                                relation = { association: array[j], attributes: [], distinct: true, subQuery: false };
                            } else {
                                relation = { association: array[j], attributes: [], distinct: true, subQuery: false };
                            }
                        } else {
                            if (arrayIshasMany[j].isHasOne || (!arrayIshasMany[j].isHasOne && !arrayIshasMany[j].isHasMany)) {
                                relation = { association: array[j], attributes: [], subQuery: false, distinct: true, include: [relation] };
                            } else {
                                relation = { association: array[j], attributes: [], subQuery: false, distinct: true, include: [relation] };
                            }
                        }
                        ...
                        if (ArrayRelationsModel[h].nameRelation === obj) {
                            ...
                            if(RolePermissionAuthenticationNested.error !== ""){
                               ...
                               relations = "Forbidden"
                                ...
                                if (ArrayRelationsModel[h].typeRelation === 'hasMany') {
                                    relations.push({ association: obj, attributes: [], distinct: true, subQuery: false });
                                } else if (ArrayRelationsModel[h].typeRelation === 'belongsTo') {
                                    relations.push({ association: obj, attributes: [], distinct: true, subQuery: false });
                                } else if (ArrayRelationsModel[h].typeRelation === 'hasOne') {
                                    relations.push({ association: obj, attributes: [], distinct: true, subQuery: false });
