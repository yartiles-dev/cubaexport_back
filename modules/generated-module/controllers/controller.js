...
// TENGO QUE TENER SIEMPRE CLARO Q SI UN USUARIO NADA MAS TIENE PERMISO SOBRE SI
// SEA PA LO QUE SEA NADA MAS PUEDE ENTRAR A LOS SERVICIOS FINDBYID, UPDATE, DELETE, CREATE.
// NADA MULTIPLE
...
const controller = class Controller {
    ...
    findById = async (req, res, next) => {
        ...
        var JsonApi = await this.jsonAPI.buildQueryFromReqSingleCollection(req, nameFolderModule, this.model.tableName);
            ...
            .then(model => {
                if (!model) {
                    ...
                    return res.status(404)
                        .json(jsonAPI.processErrors(error, req, {file: __filename}));
                    ; // Not Found.
                } else if (req.canAll === "no") {
                    let idPerson = model.hasOwnProperty("personId") ? "personId" : "id"
                    if (model[idPerson] !== req.loggedUser.id)
                        return res.status(403).json({
                            errors: [{
                                field: "Permission",
                                title: "Forbidden"
    ...
    findByParameters = async (req, res, next) => {

        if (req.canAll === "no")
            if (this.model.tableName === 'person')
                return res.status(403).json({
                    errors: [{
                        field: "Permission",
                        title: "Forbidden"
        ...
        // console.log(find)
        if (find) {
            var JsonApi = await this.jsonAPI.buildQueryFromReqFirstRequest(req, nameFolderModule, this.model.tableName);
            if (JsonApi === "Forbidden") {
                return res.status(403).json({
                    errors: [{
                        field: "Permission",
            ...
            if (req.canAll === "no") {
                if (this.model.tableName === 'person')
                    JsonApi.query.where.id = req.loggedUser.id
                else
                    JsonApi.query.where.personId = req.loggedUser.id
            }
            this.model.findAndCountAll(JsonApi.query).then(async (models) => {
                var JsonApiSecond = await this.jsonAPI.buildQueryFromReqSecondRequest(req, nameFolderModule, this.model.tableName, models.rows);
                var jsonAPIBody = {
                    data: [],
                    _meta: {}
                };
                this.model
                    ...
                    .then(model => {
                        ...
                        jsonAPIBody._meta = {
                            ...
                            pageCount: req.body?.pagination === 'all' ? 1 : Math.ceil(models.count / JsonApi.response.perPage),
                            currentPage: req.body?.pagination === 'all' ? 0 : JsonApi.response.currentPage,
                            perPage: req.body?.pagination === 'all' ? models.rows.length : JsonApi.response.perPage
                        }
                        res.status(200).json(jsonAPIBody);
                    })
                    .catch(global.app.orm.Sequelize.ValidationError, function (error) {
            ...
            var JsonApi = await this.jsonAPI.buildQueryFromReq(req, nameFolderModule, this.model.tableName);
            if (JsonApi === "Forbidden") {
                return res.status(403).json({
                    errors: [{
                        field: "Permission",
            ...
            this.model
                ...
                .then(model => {
                    ...
                    jsonAPIBody._meta = {
                        ...
                        pageCount: req.body?.pagination === 'all' ? 1 : Math.ceil(model.count / JsonApi.response.perPage),
                        currentPage: req.body?.pagination === 'all' ? 0 : JsonApi.response.currentPage,
                        perPage: req.body?.pagination === 'all' ? model.rows.length : JsonApi.response.perPage
                    }
                    res.status(200).json(jsonAPIBody);
                })
                .catch(global.app.orm.Sequelize.ValidationError, function (error) {
    ...
    delete = (req, res, next) => {
        this.model
            ...
            .then(model => {
                ...
                let isForce = this.model.name !== "serverfunction"

                model
                    .destroy({force: isForce})
                    .then(() => {
    ...
    delete_multiple = (req, res, next) => {
                    ...
                    if (!(req.body[i] instanceof Object)) {
                        ...
                        for (let i in req.body.where) {
                            if (req.body.where[i] instanceof Object) {
                                if (!req.body.where[i] instanceof Array) {
                                    find = true
                                }
                            }
                        }
                        if (find) {
                            error.push(`Los campos solicitados para filtrar no pueden ser objetos`)
                        }
                    }
                }
            }
        ...
        if (req.body.ids) {
            ...
            for (let whereKey in where) {
                if (where[whereKey] instanceof Array) {
                    where[whereKey] = {[Op.in]: where[whereKey]}
                }
            }
        }
    ...
    update = (req, res, next) => {
        ...
        // Hay que eliminar el id pq sino explotin
        delete req.body.id
        ...
        this.model
            ...
            .then(model => {
                if (!model) {
                    const error = new Error(
                        `No se encuentra el ${this.model.name} especificado para modificar`
                    );
                    error.statusCode = 404;
                    throw error;
                } else if (req.canAll === "no") {
    ...
    update_multiple = async (req, res, next) => {
        ...
        if (objs) {
            ...
            // Hay que eliminar el id del objeto pq sino explotin
            delete req.body.id
            ...
            if (!req.body.where instanceof Object) {
                ...
                if (find) {
                    error.push(`Cada parametro del objeto where debe ser string, int, date, etc. no objeto`)
                }
            }
